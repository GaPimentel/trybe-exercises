...
for (let i = 0; i < allLis.length; i += 1) {
  allLis[i].addEventListener("click", clickClass);
};
...
input.addEventListener('input', function (event) {
  const textTech = document.querySelector('.tech');
  textTech.textContent = event.target.value;
})
...
myWebpage.addEventListener('dblclick', function () {
  window.location.href = "https://gapimentel.github.io/";
})
